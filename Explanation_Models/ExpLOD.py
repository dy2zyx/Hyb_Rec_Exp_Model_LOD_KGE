...
class ExpLOD:
    ...
    """
    def __init__(self, recommender='cbf', exp_pattern='basic', nb_po=3, nb_rec=1, input_dict=None, alpha=0.5, beta=0.5):
    ...
    def basic_patterns(self, top_k_properties, filter_profil, filter_rec):
        patterns_dict = dict()
    ...
    def broader_patterns(self, top_k_properties, filter_profil, filter_rec):
        patterns_dict = dict()
    ...
    def basic_exp_generator(self):
        ...
        top_properties = list({k: v for k, v in sorted(basic_p_scores.items(), key=lambda item: item[1], reverse=True)})[:self.nb_po]
        # print("The top properties are: {}".format(top_properties))
        # generate patterns for explanation
        if len(top_properties) == 0:
            patterns_dict = dict()
        else:
            patterns_dict = self.basic_patterns(top_properties, filter_profil, filter_rec)

        return patterns_dict

    def broader_exp_generator(self):
        # generate queries
        filter_profil, filter_rec, query_basic_builder, query_broader_builder = self.generate_queries()
        # filter properties to get overlap ones
        candidate_properties_basic = self.basic_builder(query_basic_builder)
        # scoring these properties
        basic_p_scores = self.rank_p_basic(candidate_properties_basic, filter_profil, filter_rec)
        # filter properties to get overlap ones
        candidate_properties_broader = self.broader_builder(query_broader_builder)
        # scoring these properties
        broader_p_scores = self.rank_p_broader(candidate_properties_broader, basic_p_scores, filter_profil, filter_rec)
        # extract the top-k properties
        top_properties = list({k: v for k, v in sorted(broader_p_scores.items(), key=lambda item: item[1], reverse=True)})[:self.nb_po]
        # print("The top properties are: {}".format(top_properties))
        # generate patterns for explanation
        if len(top_properties) == 0:
            patterns_dict = dict()
        else:
            patterns_dict = self.broader_patterns(top_properties, filter_profil, filter_rec)

        return patterns_dict

    def exp_generator(self):
        if self.exp_pattern == 'basic':
            patterns_dict = self.basic_exp_generator()
            return patterns_dict
        elif self.exp_pattern == 'broader':
            patterns_dict = self.broader_exp_generator()
            return patterns_dict
        else:
            print("Warning, explantion pattern should be in 'basic' or 'broader'")
